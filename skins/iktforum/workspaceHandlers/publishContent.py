...
for tm_serial in contentlist:
    ...
    if topic:
        selectedcount += 1
        try:
            context.portal_workflow.doActionFor(topic,'publish') 
            successcount += 1
        except Exception,e: # happens if the user tries an unsupported workflow action (like publishing an already published element)
            errors.append(u"""Kunne ikke publisere "%s". Feilmeldingen fra systemet var: "%s" """ % (topic.title_or_id(),e,))

if successcount > 0:
    messages.append('%d element(er) ble publisert' % (successcount,))
else:
    if selectedcount == 0:
